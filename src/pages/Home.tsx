import { Suspense, lazy, useState, useEffect, useRef, createContext, useContext } from 'react';
import { Canvas } from '@react-three/fiber';
import { Plane, Globe, Shield, BookOpen, Radio, Users, Award, ChevronDown, MapPin, Phone, Mail, Menu, X, CheckCircle, Navigation, Cloud, Compass, Zap, GraduationCap, Briefcase, Building, ChevronRight, ArrowRight, Star, Clock } from 'lucide-react';
import { translations, Lang } from '../translations';

const AircraftScene = lazy(() => import('../components/AircraftScene'));
...
const langLabels: Record<Lang, { flag: string; label: string }> = {
  ...
  so: { flag: '\u{1F1F8}\u{1F1F4}', label: 'SO' },
  ar: { flag: '\u{1F1F8}\u{1F1E6}', label: 'AR' },
};
...
function LangSwitcher() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-40 rounded-xl border border-[rgba(75,142,245,0.2)] shadow-2xl overflow-hidden z-50 glass-strong" style={{ background: 'rgba(4,11,26,0.98)' }}>
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'nav-glass' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between h-[72px]">
        ...
        <div className="hidden md:flex items-center gap-3">
          <LangSwitcher />
          <a href="#contact" className="btn-primary text-xs font-bold px-5 py-2.5 inline-flex items-center gap-1.5">
      ...
      <div className={`mobile-menu md:hidden transition-all duration-300 ease-out ${menuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-6 py-6 flex flex-col gap-1">
          ...
          <a href="#contact" onClick={() => setMenuOpen(false)} className="btn-primary mt-4 text-sm font-bold px-6 py-3 text-center inline-flex items-center justify-center gap-2">
...
function Hero() {
  ...
  return (
    <section id="hero" className="hero-bg relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-4 pt-20 overflow-hidden">
      {/* 3D Aircraft background */}
      <div className="absolute inset-0 z-0 pointer-events-auto" style={{ pointerEvents: 'none' }}>
    ...
    <section id="about" className="section-alt">
      <div className="max-w-6xl mx-auto px-4 py-24">
        <div className="text-center mb-16 reveal">
          <span className="label-badge mb-4 inline-block">{t('about_label')}</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold gradient-text-light mb-5 mt-3">{t('about_title')}</h2>
          <div className="section-divider-wide w-40 mx-auto mb-5" />
          <p className="text-[#94aed4] max-w-2xl mx-auto text-base md:text-lg leading-relaxed">{t('about_subtitle')}</p>
        </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-6">
              {highlights.map((item, i) => (
                <div key={i} className="flex items-center gap-3 p-3.5 rounded-xl bg-[rgba(14,35,80,0.5)] border border-[rgba(75,142,245,0.12)] transition-all duration-300 hover:border-[rgba(122,184,247,0.3)] hover:bg-[rgba(20,50,100,0.5)]">
                  <div className={`icon-box w-9 h-9 ${item.color} shrink-0`}>{item.icon}</div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-24">
        ...
        <div ref={ref} className="reveal">
          <div className="mission-card animated-border p-10 md:p-14 text-center relative">
            ...
            <blockquote className="text-lg md:text-2xl font-semibold text-[#dceeff] leading-relaxed mb-8 italic relative">{t('mission_quote')}</blockquote>
            <p className="text-[#94aed4] text-base leading-relaxed max-w-3xl mx-auto">
              {descParts.map((part, i) => (i % 2 === 1 ? <span key={i} className="text-[#7ab8f7] font-semibold">{part}</span> : part))}
            </p>
            ...
            <div className="flex flex-wrap justify-center gap-6 md:gap-10">
              {values.map((v, i) => (
                <div key={i} className="flex items-center gap-2.5 text-xs font-bold tracking-[0.15em] uppercase text-[#94aed4]">
                  <span className={`${v.color} w-7 h-7 rounded-lg flex items-center justify-center bg-[rgba(10,26,56,0.6)] border border-[rgba(75,142,245,0.15)]`}>{v.icon}</span>
                  {t(v.key)}
                </div>
              ))}
            </div>
...
function TrainingCard({ keys, index }: { keys: { title: keyof typeof translations['en']; desc: keyof typeof translations['en'] }; index: number }) {
  const { t } = useLang();
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={ref} className="reveal" style={{ transitionDelay: `${index * 40}ms` }}>
      <div className="card-modern h-full p-6 flex flex-col gap-4 group">
        <div className={`icon-box w-14 h-14 ${color} transition-transform duration-300 group-hover:scale-110`}>{icon}</div>
        <div>
...
function Training() {
  const { t } = useLang();
  const trainingKeys = [
    { title: 't1_title' as const, desc: 't1_desc' as const }, { title: 't2_title' as const, desc: 't2_desc' as const },
    { title: 't3_title' as const, desc: 't3_desc' as const }, { title: 't4_title' as const, desc: 't4_desc' as const },
    { title: 't5_title' as const, desc: 't5_desc' as const }, { title: 't6_title' as const, desc: 't6_desc' as const },
    { title: 't7_title' as const, desc: 't7_desc' as const }, { title: 't8_title' as const, desc: 't8_desc' as const },
    { title: 't9_title' as const, desc: 't9_desc' as const }, { title: 't10_title' as const, desc: 't10_desc' as const },
  ];
  ...
  return (
    <section id="training" className="section-alt">
      <div className="max-w-7xl mx-auto px-4 py-24">
        <div className="text-center mb-16 reveal">
          <span className="label-badge mb-4 inline-block">{t('training_label')}</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold gradient-text-light mb-5 mt-3">{t('training_title')}</h2>
          <div className="section-divider-wide w-40 mx-auto mb-5" />
          <p className="text-[#94aed4] max-w-2xl mx-auto text-base md:text-lg leading-relaxed">{t('training_subtitle')}</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
          {trainingKeys.map((keys, i) => <TrainingCard key={i} keys={keys} index={i} />)}
        </div>
        <div className="mt-16 text-center">
          <div className="inline-flex items-center gap-3 px-6 py-3.5 rounded-full glass border border-[rgba(36,114,232,0.2)]">
            ...
            <span className="text-sm font-semibold tracking-[0.1em] text-[#7ab8f7] uppercase">ASECNA · ICAO WACAF Office Partner</span>
            <Globe className="text-[#4b8ef5]" size={18} />
          </div>
        </div>
      </div>
    ...
    <div ref={ref} className="reveal" style={{ transitionDelay: `${index * 60}ms` }}>
      <div className="card-modern h-full p-6 group">
        <div className="flex items-start gap-4 mb-5">
          <div className={`icon-box w-11 h-11 ${item.color} shrink-0 mt-0.5 transition-transform duration-300 group-hover:scale-110`}>{item.icon}</div>
...
function Careers() {
  ...
  return (
    <section id="careers" className="section-base">
      <div className="max-w-6xl mx-auto px-4 py-24">
        <div className="text-center mb-16 reveal">
          <span className="label-badge mb-4 inline-block">{t('careers_label')}</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold gradient-text-light mb-5 mt-3">{t('careers_title')}</h2>
          <div className="section-divider-wide w-40 mx-auto mb-5" />
          <p className="text-[#94aed4] max-w-2xl mx-auto text-base md:text-lg leading-relaxed">{t('careers_subtitle')}</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {careerData.map((item, i) => <CareerCard key={i} item={item} index={i} />)}
        </div>
      </div>
    </section>
  );
  ...
  return (
    <div ref={ref} className="reveal" style={{ transitionDelay: `${index * 80}ms` }}>
      <div className="card-modern h-full p-8 text-center flex flex-col items-center group">
        <div className={`icon-box w-16 h-16 ${item.color} mb-5 transition-transform duration-300 group-hover:scale-110`}>{item.icon}</div>
...
function WhyUs() {
  ...
  return (
    <section id="why-us" className="section-alt">
      <div className="max-w-6xl mx-auto px-4 py-24">
        <div className="text-center mb-16 reveal">
          <span className="label-badge mb-4 inline-block">{t('why_label')}</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold gradient-text-light mb-5 mt-3">{t('why_title')}</h2>
          <div className="section-divider-wide w-40 mx-auto mb-5" />
          <p className="text-[#94aed4] max-w-2xl mx-auto text-base md:text-lg leading-relaxed">{t('why_subtitle')}</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 mb-16">
          {reasonData.map((item, i) => <WhyCard key={i} item={item} index={i} />)}
        </div>
        <div ref={ctaRef} className="reveal">
          <div className="mission-card animated-border px-8 py-14 text-center">
            <p className="text-xs font-bold tracking-[0.35em] text-[#4b8ef5] uppercase mb-4">Training The Sky Professionals of Tomorrow</p>
            <h3 className="text-2xl md:text-3xl font-extrabold gradient-text-light mb-4">Build Your Strong Foundation</h3>
            <p className="text-[#94aed4] text-base max-w-xl mx-auto mb-8 leading-relaxed">The cockpit is waiting, but the journey starts in the classroom. Enrollment is open for aspiring pilots and aviation enthusiasts.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="#contact" className="btn-primary px-9 py-4 text-sm font-bold inline-flex items-center gap-2 justify-center">Register Now <ArrowRight size={16} /></a>
              <a href="tel:+252****2830" className="btn-outline px-9 py-4 text-sm font-semibold inline-flex items-center gap-2 justify-center"><Phone size={14} /> Call Us Today</a>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div ref={ref} className="reveal" style={{ transitionDelay: `${index * 80}ms` }}>
      <div className="card-modern h-full p-6 text-center group">
        <div className={`icon-box w-14 h-14 ${item.color} mx-auto mb-4 transition-transform duration-300 group-hover:scale-110`}>{item.icon}</div>
...
function Certificate() {
  ...
  return (
    <section id="certificate" className="section-base">
      <div className="max-w-5xl mx-auto px-4 py-24">
        <div className="text-center mb-16 reveal">
          <span className="label-badge mb-4 inline-block">{t('cert_label')}</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold gradient-text-light mb-5 mt-3">{t('cert_title')}</h2>
          <div className="section-divider-wide w-40 mx-auto mb-5" />
          <p className="text-[#94aed4] max-w-2xl mx-auto text-base md:text-lg leading-relaxed">{t('cert_subtitle')}</p>
        </div>
        <div ref={imageRef} className="reveal mb-12">
          <div className="relative rounded-2xl overflow-hidden p-3 bg-[rgba(10,26,56,0.6)] border border-[rgba(36,114,232,0.15)]">
            ...
            <img src="/ST.png" alt="Stratosphere Aeronautics Certificate of Completion" className="w-full rounded-xl shadow-2xl" style={{ boxShadow: '0 25px 60px rgba(0,0,0,0.5), 0 0 40px rgba(36,114,232,0.1)' }} />
          </div>
        </div>
        <div className="grid sm:grid-cols-3 gap-5">
          {certCards.map((item, i) => <CertCard key={i} item={item} index={i} />)}
        </div>
      </div>
    </section>
  );
...
function Contact() {
  ...
  return (
    <section id="contact" className="section-alt">
      <div className="max-w-6xl mx-auto px-4 py-24">
        <div className="text-center mb-16 reveal">
          <span className="label-badge mb-4 inline-block">{t('contact_label')}</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold gradient-text-light mb-5 mt-3">{t('contact_title')}</h2>
          <div className="section-divider-wide w-40 mx-auto mb-5" />
          <p className="text-[#94aed4] max-w-2xl mx-auto text-base md:text-lg leading-relaxed">{t('contact_subtitle')}</p>
        </div>
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[#94aed4] text-sm leading-relaxed">Bahsane Building, 2nd Floor, Room 213<br />Western Entrance (Facing West)<br />Opposite Ex. National Cinema<br /><span className="text-[#7ab8f7] font-semibold">{t('contact_address_val')}</span></p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a href="mailto:info@stratosphereaeronautics.com" className="block text-[#7ab8f7] hover:text-[#a8d4fb] text-sm transition-colors break-all font-medium">info@stratosphereaeronautics.com</a>
                    <a href="mailto:abdirahman.dahir@stratosphereaeronautics.com" className="block text-[#7ab8f7] hover:text-[#a8d4fb] text-sm transition-colors break-all font-medium mt-1">abdirahman.dahir@stratosphereaeronautics.com</a>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="card-modern w-full p-10 flex flex-col items-center justify-center text-center">
              ...
              <div className="mt-10 pt-8 border-t border-[rgba(36,114,232,0.1)] w-full">
                ...
                <div className="flex flex-col gap-2">
                  <a href="tel:+252****2830" className="inline-flex items-center justify-center gap-2 text-sm font-medium text-[#7ab8f7] hover:text-[#a8d4fb] transition-colors"><Phone size={14} /> +252 63 4482830</a>
                  <a href="mailto:info@stratosphereaeronautics.com" className="inline-flex items-center justify-center gap-2 text-sm font-medium text-[#7ab8f7] hover:text-[#a8d4fb] transition-colors"><Mail size={14} /> info@stratosphereaeronautics.com</a>
                </div>
              </div>
            </div>
          </div>
...
function Footer() {
  const { t } = useLang();
  const footerLinks = [
    { href: '#about', key: 'nav_about' as const }, { href: '#training', key: 'nav_training' as const },
    { href: '#careers', key: 'nav_careers' as const }, { href: '#why-us', key: 'nav_why_us' as const },
    { href: '#certificate', key: 'nav_certificate' as const }, { href: '#contact', key: 'nav_contact' as const },
  ];
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-10 mb-10">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2">
            {footerLinks.map((item, i) => (
              <a key={i} href={item.href} className="text-xs font-semibold tracking-widest text-[#4a6080] hover:text-[#7ab8f7] uppercase transition-colors duration-200">{t(item.key)}</a>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          {[
            ...
            <div key={i} className="flex items-center gap-2 px-4 py-2 rounded-full border border-[rgba(36,114,232,0.15)] bg-[rgba(36,114,232,0.05)] text-xs font-semibold text-[#4a6080]">
              <span className="text-[#2472e8]">{badge.icon}</span>{badge.label}
            </div>
          ))}
        </div>
        <div className="section-divider mb-6" />
...
function StarsBackground() {
  const stars = Array.from({ length: 100 }, (_, i) => ({
    id: i, left: `${(i * 13 + 7) % 100}%`, top: `${(i * 17 + 3) % 100}%`,
    size: (i % 3) * 0.8 + 0.6, duration: `${(i % 4) + 2.5}s`, delay: `${(i % 5) * 0.8}s`,
  }));
  return (
    <div className="stars-bg">
      {stars.map(s => (
        <div key={s.id} className="star" style={{ left: s.left, top: s.top, width: `${s.size}px`, height: `${s.size}px`, '--duration': s.duration, '--delay': s.delay } as React.CSSProperties} />
      ))}
    </div>
  );
}
...
/* ── Lang provider wrapper ─────────────────────────────────────────────────────── */
export function HomeWithLang() {
